...
import { floor1, floor10 } from "@/domain/rounding";
 ...
 * 기본요금은 해당 적용 구간(총 사용량이 속한 구간)의 baseFee, marginalRate는 그 구간의 단가.
...
describe("누진요금 계산 엔진 calcBill + 반올림 유틸", () => {
  describe("반올림 유틸 (floor1, floor10)", () => {
    it("floor1: 소수점 첫째 자리까지만 남기고 내림", () => {
      expect(floor1(100.56)).toBe(100.5);
      expect(floor1(100.99)).toBe(100.9);
      expect(floor1(100.0)).toBe(100.0);
      expect(floor1(100.1)).toBe(100.1);
    });

    it("floor10: 십의 자리까지만 남기고 내림", () => {
      expect(floor10(1234.56)).toBe(1230);
      expect(floor10(1299)).toBe(1290);
      expect(floor10(1200)).toBe(1200);
      expect(floor10(1209)).toBe(1200);
    });
  });
  ...
  describe("AC-1: 통상 요금 (5월, 300kWh, 저압)", () => {
    it("AC-1[P0]: calcBill({kWh:300, yearMonth:'2026-05', contractType:'low'}) → 구체적인 값 검증", () => {
      ...
      const result: BillBreakdown = calcBill(input);

      // 기본요금: 300kWh는 2구간(200~400)에 속함 → 2구간 기본요금
      ...
      // 기후환경요금: 300 * 9.0(원/kWh) = 2700
      expect(result.climateFee).toBe(2700);

      // 연료비조정요금: 300 * 5.0(원/kWh) = 1500
      ...
      // 에너지기금: 51260 * 3.7% = 1896.62 → floor10 = 1890
      expect(result.fund).toBe(1890);

      // 합계: floor10(51260 + 5126 + 1890) = floor10(58276) = 58270
  ...
  describe("AC-2: 여름 요금 적용 & 요금 차이", () => {
    it("AC-2[P0]: 8월은 isSummerRelief=true, 5월은 false", () => {
      const summer = calcBill({
        kWh: 450,
        yearMonth: "2026-08",
        contractType: "low",
      });
      const normal = calcBill({
        kWh: 450,
        yearMonth: "2026-05",
        contractType: "low",
      });
  ...
  describe("AC-3: 3구간 배분 & 한계요금", () => {
    ...
    it("AC-3: 200kWh 시 1구간만 사용", () => {
      ...
      // 한계요금 & 기본요금: 1구간 기준 (200kWh는 1구간 상한에 정확히 도달, 2구간 미진입)
      expect(result.marginalRate).toBe(120.0);
      expect(result.baseFee).toBe(910);
    });
    ...
    it("AC-3: 400kWh 시 2구간까지 사용, 3구간 미사용", () => {
      ...
      expect(result.marginalRate).toBe(214.6);
      expect(result.baseFee).toBe(1600); // 2구간 기본요금
    });
  });
  ...
  describe("AC-4: 고압 요금 (contractType='high')", () => {
    it("AC-4[P0]: 300kWh 고압 요금 검증", () => {
      ...
      // 첫 구간 단가 확인 (tariff.ts 기준 고압 1구간 단가)
      expect(result.tiers[0].rate).toBe(105.0);
    });
  });
